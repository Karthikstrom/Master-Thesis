...
class CriticNetwork(keras.Model):
    def __init__(self,fc1_dims=64,fc2_dims=124,fc4_dims=32,name='critic',chkpt_dir='tmp/ddpg'):
        ...
        self.checkpoint_file=os.path.join(self.checkpoint_dir,self.model_name+'_ddpg_V7.h5')
...
class ActorNetwork(keras.Model):
    def __init__(self,fc1_dims=64,fc2_dims=124,fc4_dims=32,n_actions=2,name='actor',chkpt_dir='tmp/ddpg'):
...
class Agent:
    def __init__(self,input_dims,alpha=0.00005,beta=0.00001,env=None,gamma=0.99,
                 n_actions=2,max_size=50000,tau=0.05,batch_size=32,no_of_episodes=7000):
        ...
        self.a1_noise=self.Gaussian_Noise(0.3)
        self.a2_noise=self.Gaussian_Noise(0.1)
    ...
    def Gaussian_Noise(self,sig):
        # Parameters for Gaussian noise
        mu = 0.0  # Mean of the noise
        sigma_initial = sig # Initial standard deviation of the noise
        sigma_final =sig # Final standard deviation of the noise
        dt = 1  # Time step
        T = (self.no_of_episodes+1)*24  # Total time
    ...
    def choose_action(self,observation,evaluate=False):
        ...
        #print(actions)
        noise_tens=[self.a1_noise[self.step_counter],self.a2_noise[self.step_counter]]
        ...
        if not evaluate:
            ...
            self.step_counter+=1
            actions_noise = tf.clip_by_value(actions_noise,min_act,max_act)
            return actions_noise
        else:
            actions= tf.clip_by_value(actions,min_act,max_act)
            return actions
...
if __name__ == '__main__':
    ...
    for i in range(n_games):
        ...
        while not done:
            ...
            #print(action)
            observation_,reward,done,r=env.step(action[0],action[1])
            agent.remember(observation,action,reward,observation_,done)
...
for i in range(test_episode):
        ...
        while not done:
            ...
            action=np.reshape(action,(2,1))
            print(action)
            new_state,reward,done,_=env.step(action[0],action[1])
...
import pandas as pd
df_results = pd.DataFrame({'Load':load, 'PV': pv,'Bat_action':a1,'RTP':price,'pp':grid_p,'SOC':batt_soc})
fig,ax=plt.subplots()
