...
#Calling the function with historic data
df=real_load()
...
class EMSenv(gym.Env):
    ...
    def __init__(self):
        ...
        #Battery grid exchange
        self.grid_bat_imp_min=0
        self.grid_bat_imp_max=2.8
        
        self.grid_bat_exp_min=0
        self.grid_bat_exp_max=2.8
        ...
        self.obs_low=np.array([self.min_load,self.min_price,self.min_pv,self.min_hour,
                               ...
                               self.ps_min,self.pp_min,self.grid_p_low,self.grid_bat_imp_min,self.grid_bat_exp_min])
        ...
        self.obs_high=np.array([self.max_load,self.max_price,self.max_pv,self.max_hour,
                                ...
                                self.ps_max,self.pp_max,self.grid_p_high,self.grid_bat_imp_max,self.grid_bat_exp_max])
        ...
        self.act_low=np.array([self.a1_min,self.a2_min])
        self.act_high=np.array([self.a1_max,self.a2_max])
    ...
    def reset(self):
        ...
        grid_bat_imp=round(self.rng.uniform(self.grid_bat_imp_min, self.grid_bat_imp_max), 3)
        grid_bat_exp=round(self.rng.uniform(self.grid_bat_exp_min, self.grid_bat_exp_max), 3)
        ...
        self.current_obs=np.array([self.load[0],self.price[0],self.pv[0],self.hour_feature[0],
                                  ...
                                  soc,pb_in,pb_out,pb_imp,pb_exp,ps,pp,grid_p,grid_bat_imp,grid_bat_exp])
    ...
    def step(self,a1,a2):
        ...
        # making invalid actions zero
        
        #if a1 is positive, battery is importing so exp to grid is zero and vice versa
        #this is only regarding the exchange with the grid (a1) - can be visually seen in the figure I have attached
        if a1>0:
            pb_imp_grid_temp=a1
            pb_exp_grid_temp=0
        elif a1<0:
            #taking the abosolute value of discharge for easier calculation purpose
            pb_exp_grid_temp=abs(a1)
            pb_imp_grid_temp=0
        else:
            pb_exp_grid_temp=0
            pb_imp_grid_temp=0
        ...
        # a2 is always positive i.e although discharge to load is negative it is kept positive for
        # easier calculation purposes
        
        # if the battery is importing from the grid then it cant export to the battery at the same time 
        # so making it zero
        if pb_imp_grid_temp>0:
            pb_exp_load_temp=0
        else:
            pb_exp_load_temp=a2
        
        
        #computing run-time calculations
        
        
        #Total battery import and export
        pb_imp_temp=pb_imp_grid_temp
        pb_exp_temp=pb_exp_grid_temp+pb_exp_load_temp
        
        #grid purchase afer the load gets from battery
        grid_p=self.current_obs[0]-pb_exp_load_temp
        
        
        #Total electricity sold is battery discharged to the grid
        ps=pb_exp_grid_temp
        
        #Total electricity purchased
        pp=grid_p+pb_imp_grid_temp
        
        #Battery action (if charging its +ive else negative)
        bat_action=pb_imp_temp-pb_exp_temp
        ...
        #Updating state of historic values
        ...
        if (next_soc<=self.soc_min):
            reward_1=-abs(self.soc_min-next_soc)
        else:
            reward_1=0
            
        if (next_soc>=self.soc_max):
            reward_2=-abs(next_soc-self.soc_max)
        else:
            reward_2=0
        
        
        #Increase financial benefit/ reduce the net electricity cost
        reward_3=-(pp-ps)*next_load
        
        
        #Making sure the grid purchase is only positive and not selling back
        if grid_p<0:
            reward_4=grid_p
        else:
            reward_4=0
        ...
        reward=10*reward_1+10*reward_2+reward_3+10*reward_4
        ...
        next_obs=[next_load,next_price,next_pv,next_hour,next_3h_price,
                  ...
                  next_soc,next_pb_in,next_pb_out,pb_imp_temp,pb_exp_temp,ps,pp,grid_p,pb_imp_grid_temp,pb_exp_grid_temp]
...
for i in range(1):
    ...
    while done==False:
        ...
        a1 = action[0]
        a2 = action[1]
        obs,r,done,r=test_env.step(a1,a2)
