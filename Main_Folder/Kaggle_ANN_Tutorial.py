# -*- coding: utf-8 -*-
"""
Created on Mon Dec 19 09:54:47 2022
...
#from essentialfunctions import metrics
...
scaler.fit(df)
#df['grid_import']=scaler.transform(df)
#%% Train and test split
load=df.loc[:'2019-04-30']
test_load=df.loc['2019-05-01':'2019-05-01']
#%% Functions
...
def preprocess_FNN(data, look_back):
    ...
    for i in range(data.shape[0]-look_back):
        ...
        # why would want this in the reverse direction?
        # most recent one the last?
        y = data[look_back+i]
        X_train.append(list(x))
        y_train.append(y)
    input_seq_for_test = data[i+1:look_back+i+1][::-1]# the final look back array
...
def forecast_FNN(model, input_sequence, future_steps):
    ...
    for i in range(future_steps):
        ...
        input_sequence[0] = np.append(forecasted_value, input_sequence[0][:-1])#sorting might be the optimum but yar
...
def FNN(data, look_back, hidden_nodes, output_nodes, epochs, batch_size, future_steps, scaler):
    ...
    forecasted_values_FNN = list(scaler.inverse_transform([forecasted_values_FNN])[0])# 0 cause its 2D array everywhere
    
    return model_FNN, forecasted_values_FNN

#%% Get scores on test data

def get_accuracies_FNN(load, test_load, parameters, scaler):
    ...
    for param in combination_of_params:
        ...
        model_FNN, forecasted_values_FNN = FNN(load, look_back, hidden_nodes, output_nodes, epochs, batch_size, future_steps, scaler)
        
        y_true = test_load.ix[:future_steps].grid_import
...
def get_accuracies_TLNN(load, test_load, parameters, scaler):
    combination_of_params = get_combinations(parameters)
    information_TLNN = []
    iterator = 0
    ...
    for param in combination_of_params:
        ...
        model_TLNN, forecasted_values_TLNN = TLNN(load, test_load, hidden_nodes, output_nodes, epochs, batch_size, future_steps, scaler)
...
def get_accuracies_LSTM(load, test_load, parameters, scaler):
    combination_of_params = get_combinations(parameters)
    information_LSTM = []
    iterator = 0
    ...
    for param in combination_of_params:
        ...
        model_LSTM, forecasted_values_LSTM = Long_Short_Term_Memory(load, input_nodes, hidden_nodes, output_nodes, epochs, batch_size, future_steps, scaler)
...
#%% Analyzing the test data and forecasted data

def analyze_results(data_frame, test_load, name, flag=False):
    ...
    y_true = test_load.ix[:int(future_steps)]
...
def compare_ANN_methods(load, test_load, scaler, parameters_FNN, parameters_TLNN, parameters_SANN, parameters_LSTM, future_steps):
    
    information_FNN_df = get_accuracies_FNN(load, test_load, parameters_FNN, scaler)
    optimized_params_FNN = analyze_results(information_FNN_df, test_load, 'FNN')
    
    information_TLNN_df = get_accuracies_TLNN(load, test_load, parameters_TLNN, scaler)
    optimized_params_TLNN = analyze_results(information_TLNN_df, test_load, 'TLNN')
    
    information_SANN_df = get_accuracies_SANN(load, test_load, parameters_SANN, scaler)
    optimized_params_SANN = analyze_results(information_SANN_df, test_load, 'SANN')
    
    information_LSTM_df = get_accuracies_LSTM(load, test_load, parameters_LSTM, scaler)
    optimized_params_LSTM = analyze_results(information_LSTM_df, test_load, 'LSTM')
    ...
    best_optimized_params = analyze_results(information[index], test_load, name, True)
    return RMSE_info

future_steps = 24

# look_back, hidden_nodes, output_nodes, epochs, batch_size, future_steps
parameters_FNN = [[24], [3,4,5,6], [1], [500], [20], [future_steps]]

# time_lagged_points, hidden_nodes, output_nodes, epochs, batch_size, future_steps
parameters_TLNN = [[[24], [1,2,3,4,11,12], [1,2,3,11,12,13], [1,2,3,4,5,6,10,11,12]], [3,4,5,6], [1], [300], [20], [future_steps]]

# seasonal_period, hidden_nodes, epochs, batch_size, future_steps
parameters_SANN = [[24], [3,4,5,6,7,8,9,10], [500], [20], [future_steps]]

# look_back, hidden_nodes, output_nodes, epochs, batch_size, future_steps
parameters_LSTM = [[24], [3,4,5,6], [1], [300], [20], [future_steps]]

RMSE_info = compare_ANN_methods(load, test_load, scaler, 
