...
class EMSenv(gym.Env):
    ...
    def step(self,a1,a2):
        ...
        if (self.current_obs[12]<=self.soc_min):
            reward_1=-3*abs(self.soc_min-self.current_obs[12])
            
        elif (self.current_obs[12]>=self.soc_max):
            reward_1=-3*(self.current_obs[12]-self.soc_max)
        
        else:
            reward_1=0
        
        reward_2=-3*abs(self.current_obs[0]-load_total)
            
        
        reward_3=-3*abs((pp-ps)*self.current_obs[1])
        ...
        next_pb_in=np.round(next_pb_in, 3)
        next_pb_out=self.pb_out_func(next_soc)
        next_pb_out=np.round(next_pb_out, 3)
        pb_imp_temp=np.round(pb_imp_temp, 3)
        pb_exp_temp=np.round(pb_exp_temp,3)
