...
df['RTP']=df['RTP'].astype(np.float32)
df=df[-8760:]
df1=df.copy()
...
plot_acf(df['RTP'],lags=100)
plot_pacf(df['RTP'],lags=100)
autocorrelation_plot(df['RTP'])
...
best_model=sm.tsa.statespace.SARIMAX(train['RTP'], order=(p, d, q), 
...
test=test['2019-05-02':'2019-05-04']
#fig,ax=plt.subplots(figsize=(10,5))
fig,ax=plt.subplots(figsize=(12,7.35))
ax.plot(test['RTP'],label="Actual",color='b')
...
metrics(test['RTP'],test['predicted'])
