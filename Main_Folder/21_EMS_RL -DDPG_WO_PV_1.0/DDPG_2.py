...
class CriticNetwork(keras.Model):
    def __init__(self,fc1_dims=64,fc2_dims=124,fc4_dims=32,name='critic',chkpt_dir='tmp/ddpg'):
        ...
        self.checkpoint_file=os.path.join(self.checkpoint_dir,self.model_name+'_ddpg_V6.h5')
...
class Agent:
    def __init__(self,input_dims,alpha=0.00005,beta=0.00001,env=None,gamma=0.99,
                 n_actions=2,max_size=50000,tau=0.05,batch_size=32,no_of_episodes=5000):
    ...
    def Gaussian_Noise(self):
        ...
        mu = 0.0  # Mean of the noise
        sigma_initial = 1  # Initial standard deviation of the noise
    ...
    def choose_action(self,observation,evaluate=False):
        ...
        #     actions_noise = actions_noise.numpy()
        #     #print(actions_noise.shape)
        #     actions_noise = actions_noise.flatten()
        #     #restricting a1 values from exploring negative values
        #     if actions_noise[0]<0:
        #         actions_noise[0]=0
        #     else:
        #         actions_noise[0]=actions_noise[0]
        #     self.step_counter+=1
...
for i in range(test_episode):
        ...
        while not done:
            ...
            load_tot_list.append(a1+abs(current_state[16]))
...
import pandas as pd
df_results = pd.DataFrame({'Load':load, 'PV': pv,'Grid_exh':a1,'Bat_action':a2})
fig,ax=plt.subplots()
ax.plot(df_results['Load'])
#ax.plot(df_results['l_tot'])
ax.plot(df_results['Bat_action'])
plt.show()
