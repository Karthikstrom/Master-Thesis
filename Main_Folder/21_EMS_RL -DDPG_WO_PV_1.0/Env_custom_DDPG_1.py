...
class EMSenv(gym.Env):
    ...
    def reset(self):
        ...
        self.current_obs=np.array([self.load[0],self.price[0],self.pv[0],self.hour_feature[0],
                                  ...
                                  soc,pb_in,pb_out,pb_imp,pb_exp,ps,pp])
    ...
    def step(self,a1,a2):
        ...
        elif (self.current_obs[12]>=self.soc_max):
            reward_1=-(self.current_obs[12]-self.soc_max)
        ...
        #print(reward_1,reward_2,reward_3,reward_4)
        ...
        # self.r1_min = min(self.r1_min, reward_1)
        # self.r1_max = max(self.r1_max, reward_1)
        
   
        # reward_1=(reward_1-self.r1_min)/(self.r1_max-self.r1_min)
        
        
        # self.r2_min = min(self.r2_min, reward_2)
        # self.r2_max = max(self.r2_max, reward_2)
        
        # reward_2=(reward_2-self.r2_min)/(self.r2_max-self.r2_min)
        
        
        
        # self.r3_min = min(self.r3_min, reward_3)
        # self.r3_max = max(self.r3_max, reward_3)
        # reward_3=(reward_3-self.r3_min)/(self.r3_max-self.r3_min)
        
        
        # self.r4_min = min(self.r4_min, reward_4)
        # self.r4_max = max(self.r4_max, reward_4)
        # reward_4=(reward_4-self.r4_min)/(self.r4_max-self.r4_min)
        
        # if self.reward_counter<5:
        #     reward_1=(reward_1-self.r1_mean)/self.r1_std
        #     reward_2=(reward_2-self.r2_mean)/self.r2_std
        #     reward_3=(reward_3-self.r3_mean)/self.r3_std
        #     reward_4=(reward_4-self.r4_mean)/self.r4_std
        
        # else:
            
        #     self.r1 = [x for x in self.r1 if ~np.isnan(x)]
        #     self.r2 = [x for x in self.r2 if ~np.isnan(x)]
        #     self.r3 = [x for x in self.r3 if ~np.isnan(x)]
        #     self.r4 = [x for x in self.r4 if ~np.isnan(x)]
            
        #     self.r1_mean=np.mean(self.r1)
        #     self.r2_mean=np.mean(self.r2)
        #     self.r3_mean=np.mean(self.r3)
        #     self.r4_mean=np.mean(self.r4)
            
        #     self.r1_std=np.std(self.r1)
        #     self.r2_std=np.std(self.r2)
        #     self.r3_std=np.std(self.r3)
        #     self.r4_std=np.std(self.r4)
        ...
        # self.r1.append(reward_1)
        # self.r2.append(reward_2)
        # self.r3.append(reward_3)
        # self.r4.append(reward_4)
        
        reward=reward_1+reward_2+1.5*reward_3+1.5*reward_4
        ...
        # next_pb_in=next_pb_in/self.pb_in_max
        # next_pb_out=next_pb_out/self.pb_out_max
        # pb_imp_temp=pb_imp_temp/self.pb_in_max
        # pb_exp_temp=pb_exp_temp/self.pb_out_max
        # #ps=ps/self.ps_max
        # pp=pp/self.pp_max
        ...
        next_obs=[next_load,next_price,next_pv,next_hour,next_3h_price,
                  ...
                  next_soc,next_pb_in,next_pb_out,pb_imp_temp,pb_exp_temp,ps,pp]
