...
model_cnn.add(Conv1D(filters=64, kernel_size=24, activation='relu', input_shape=(train_x.shape[1],train_x.shape[2])))
model_cnn.add(MaxPooling1D(pool_size=24))
model_cnn.add(Flatten())
model_cnn.add(Dense(50, activation='relu'))
model_cnn.add(Dense(future_steps))
...
#%% VMLP Predicting
y_pred_multi=model_cnn.predict(test_x)
...
day_ip=np.asarray(df.loc['2009-07-07'])
day_op=np.asarray(df.loc['2009-07-08'])


day_ip=np.reshape(day_ip,(1,24,1))
day_pred=model_cnn.predict(day_ip)
