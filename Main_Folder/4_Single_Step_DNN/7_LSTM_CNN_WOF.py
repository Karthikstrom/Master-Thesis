...
#%%LSTM model
#optimizer=Adam(learning_rate=0.000)

lstm_model=Sequential()
lstm_model.add(TimeDistributed(Conv1D(filters=16,kernel_size=1,activation='relu'),input_shape=(None,train_x.shape[2],train_x.shape[3])))
#lstm_model.add(TimeDistributed(MaxPooling1D(pool_size=2)))
lstm_model.add(TimeDistributed(Flatten()))
lstm_model.add(Dropout(0.6))
lstm_model.add(LSTM(26, activation='relu'))
lstm_model.add(Dense(op_steps))
lstm_model.compile(loss='mse', optimizer='adam')
lstm_model.summary()

lstm_history =lstm_model.fit(train_x,train_y, validation_data=(val_x, val_y), epochs=60, verbose=2)
