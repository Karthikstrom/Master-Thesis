...
for i in range(test_x.shape[0]): #first loop to run every ip sequence(24 hours)
    ...
    for j in range(24): #second loop to regressivly predict next hours in a day
            ...
            temp_x=np.reshape(temp_x,(1,12,2,1))
...
path=r"C:\Users\Karthikeyan\Desktop\Github\Master-Thesis\Main_Folder\6_Autoregressive_DNN\Plots\AR_CNNLSTM.jpeg"
...
bx.set_ylabel("Load")
plt.title("Multi Step (Regressive) CNN-LSTM Actual vs Prediction")
plt.xlim(600,1000)
plt.legend()
