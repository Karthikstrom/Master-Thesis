...
class EMSenv(gym.Env):
    ...
    def __init__(self):
        ...
        self.obs_low=np.array([self.min_load,self.min_price,self.min_pv,
                               self.min_battery_cap,self.min_hour,
        ...
        self.obs_high=np.array([self.max_load,self.max_price,self.max_pv,
                                self.max_battery_cap,self.max_hour,
    ...
    def step(self,action):
        ...
        #excess 
        if (self.current_obs[2]-self.current_obs[0])>0:
            
        
        #transported to the grid (gt)
        grid_t=self.current_obs[0]-self.current_obs[2]+battery_action
        
        #Reward for battery limits
        if (self.current_obs[3]<=self.min_battery_cap) | (self.current_obs[3]>=self.max_battery_cap):
            reward=-1
        else:
            reward=-(self.current_obs[1]*grid_t)
        ...
        next_obs=[next_load,next_price,next_pv,next_battery_cap,
                  next_hour,next_3h_price,next_6h_price,next_12h_price,next_24h_price,
